...
public class TC6 extends BaseTest1 {
 ...
 options.addArguments("--start-maximized");
    ...
    @Test(priority = 1, description = "FLOW - COMBO 199,000Đ: TẾT TƯƠI MỚI", invocationCount = 1)
    public void TC06 () throws InterruptedException {
        ...
        // Nhập mã sản phẩm combo
        String product3 = getProductCode("product8");
        ...
        // Click vào SP combo bằng JS - tìm element chứa text "TEST COMBO" hoặc giá "2,140,000"
        // Dùng JS document.querySelector để tìm chính xác trong DOM
        js.executeScript(
            "var items = document.querySelectorAll('[class*=\"search-input-dropdown\"] [class*=\"ant-select-item-option\"]');" +
        ...
        // SP Combo → hiện popup chi tiết combo → click "Thêm vào giỏ hàng"
        WebElement btnThemVaoGio = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//button[contains(.,'Thêm vào giỏ hàng') or contains(.,'Thêm vào giỏ') or contains(.,'thêm vào giỏ hàng')]")));
        js.executeScript("arguments[0].scrollIntoView({block:'center'});", btnThemVaoGio);
        Thread.sleep(500);
        js.executeScript("arguments[0].click();", btnThemVaoGio);
        Thread.sleep(3000);

        Thread.sleep(2000);

        tc08.pass("Đã nhập sản phẩm combo " + product3 + " và click Thêm vào giỏ hàng");
