...
public class TC21 extends BaseTest1 {
 ...
 options.addArguments("--start-maximized");
    ...
    @Test(priority = 1, description = "FLOW - Giảm giá sản phẩm thuộc tích HotItem với giảm 5%", invocationCount = 1)
    public void TC021 () throws InterruptedException {
         ...
         * =========================
         */
        ExtentTest tc08 = test.createNode("TC08 - Nhập sản phẩm 00002643");
        ...
        // Nhập mã sản phẩm
        String product3 = getProductCode("product_tc21");
        ...
        js.executeScript(
                ...
                "nativeInputValueSetter.call(el, '" + getProductQuantity("product_tc21") + "');" +
        ...
        try {
            ...
            // BƯỚC 1: Bấm vào "Khuyến mãi khác" để mở popup
            try {
                ...
                // Đóng popup
                try {
                    WebElement btnDong = wait.until(ExpectedConditions.elementToBeClickable(
                            By.xpath("//button[contains(.,'Đóng')] | //div[contains(@class,'ant-modal')]//span[contains(@class,'anticon-close')]/ancestor::button")));
                    btnDong.click();
                    Thread.sleep(1000);
                } catch (Exception ex) { }

            } catch (Exception e) {
                tcVerifyPrice.fail("❌ Không tìm thấy link 'Khuyến mãi khác' để mở popup: " + e.getMessage());
            }

            // BƯỚC 3: Verify giá trên màn hình bán hàng
