...
public class TC20 extends BaseTest1 {
 ...
 options.addArguments("--start-maximized");
    ...
    @Test(priority = 1, description = "FLOW - Tổng tiền đơn hàng theo sp --> số tiền giảm", invocationCount = 1)
    public void TC020 () throws InterruptedException {
         ...
         * =========================
         */
        ExtentTest tc08 = test.createNode("TC08 - Nhập sản phẩm 00031188");
        ...
        // Nhập mã sản phẩm
        String product3 = getProductCode("product_tc20");
        ...
        tc08.pass("Đã nhập sản phẩm 00031188 và chọn đơn vị Hộp");
        ...
        js.executeScript(
                ...
                "nativeInputValueSetter.call(el, '" + getProductQuantity("product_tc20") + "');" +
         ...
         * =========================
         */
        ExtentTest tcVerifyPrice = test.createNode("TC08-VERIFY - Verify giá sau khi áp CTKM nhóm hàng giảm giá");

        Thread.sleep(3000); // Đợi giá cập nhật

        try {
            // Lấy pageSource trước khi mở popup để check giá trên trang chính
            String pageSource = driver.getPageSource();

            // Debug: log các giá trị tìm thấy trên trang
            ...
            // Check Tổng tiền ban đầu = 50,000 đ (sp 00031188 × 1 hộp)
            if (pageSource.contains("50,000") || pageSource.contains("50.000")) {
                tcVerifyPrice.pass("✅ Tổng tiền ban đầu = 50,000 đ");
            } else {
                tcVerifyPrice.fail("❌ Tổng tiền ban đầu KHÔNG tìm thấy 50,000 trên trang");
            }

            // Check Giảm giá trực tiếp = 10,000 đ
            if (pageSource.contains("10,000") || pageSource.contains("10.000")) {
                tcVerifyPrice.pass("✅ Giảm giá trực tiếp = 10,000 đ");
            } else {
                tcVerifyPrice.fail("❌ Giảm giá trực tiếp KHÔNG tìm thấy 10,000 trên trang");
            }

            // Check Tạm tính = 40,000 đ
            if (pageSource.contains("40,000") || pageSource.contains("40.000")) {
                tcVerifyPrice.pass("✅ Tạm tính = 40,000 đ (đã giảm 10,000 từ CTKM)");
            } else {
                tcVerifyPrice.fail("❌ Tạm tính KHÔNG tìm thấy 40,000 trên trang");
            }
            ...
            try {
                WebElement kmKhacLink = wait.until(ExpectedConditions.elementToBeClickable(
                        By.xpath("//*[contains(text(),'Khuyến mãi khác')]")));
                js.executeScript("arguments[0].click();", kmKhacLink);
                Thread.sleep(2000);

                // Đợi popup "Danh sách khuyến mãi" xuất hiện
                wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.xpath("//*[contains(text(),'Danh sách khuyến mãi')]")));
                ...
                // Đóng popup bằng nút Xác nhận (F4)
                try {
                    ...
                    try {
                        driver.findElement(
                                By.xpath("//div[contains(@class,'ant-modal')]//span[contains(@class,'anticon-close')]/ancestor::button | " +
                                        "//div[contains(@class,'ant-modal-close')]")).click();
                        Thread.sleep(1000);
                    } catch (Exception ex2) {
                ...
                tcVerifyPrice.fail("❌ Không thể mở popup 'Khuyến mãi khác': " + e.getMessage());
            }
