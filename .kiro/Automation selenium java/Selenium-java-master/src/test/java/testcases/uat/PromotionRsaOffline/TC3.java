...
public class TC3 extends BaseTest1 {
 ...
 options.addArguments("--start-maximized");
    ...
    @Test(priority = 1, description = "FLOW - Loại hàng giảm giá", invocationCount = 1)
    public void TC03 () throws InterruptedException {
         ...
         * =========================
         */
        ExtentTest tc08 = test.createNode("TC08 - Nhập sản phẩm C042300000097");
        ...
        // Nhập mã sản phẩm
        String product3 = getProductCode("product5");
        ...
        js.executeScript(
                ...
                "nativeInputValueSetter.call(el, '" + getProductQuantity("product5") + "');" +
        ...
        try {
            ...
            // Check mã CTKM KM-0626-071 trong popup "Khuyến mãi khác"
            try {
                ...
                if (popupSource.contains("KM-0626-071")) {
                    tcVerifyPrice.pass("✅ CTKM KM-0626-071 đã được CHỌN trong popup Khuyến mãi");
                } else {
                    tcVerifyPrice.fail("❌ KHÔNG tìm thấy CTKM KM-0626-071 trong popup");
                }
                ...
                try {
                    ...
                    try {
                        driver.findElement(By.xpath("//div[contains(@class,'ant-modal')]//span[contains(@class,'anticon-close')]/ancestor::button")).click();
                        Thread.sleep(1000);
                    } catch (Exception ex2) { }
                }
            } catch (Exception e) {
            ...
            if (pageSource.contains("209,000") || pageSource.contains("209.000") || pageSource.contains("209000")) {
                tcVerifyPrice.pass("✅ Tổng tiền ban đầu = 209,000 đ");
            } else {
                tcVerifyPrice.fail("❌ Tổng tiền ban đầu KHÔNG tìm thấy 209,000 trên trang");
            }
            
            // Check Giảm giá trực tiếp
            if (pageSource.contains("7,000") || pageSource.contains("7.000")) {
                tcVerifyPrice.pass("✅ Giảm giá trực tiếp = 7,000 đ");
            } else {
                tcVerifyPrice.fail("❌ Giảm giá trực tiếp KHÔNG tìm thấy 7,000 trên trang");
            }
            
            // Check Tạm tính
            if (pageSource.contains("202,000") || pageSource.contains("202.000") || pageSource.contains("202000")) {
                tcVerifyPrice.pass("✅ Tạm tính = 202,000 đ (đã giảm 7,000 từ CTKM KM-0626-071)");
            } else {
                tcVerifyPrice.fail("❌ Tạm tính KHÔNG tìm thấy 202,000 trên trang");
            }
        } catch (Exception e) {
        ...
        test.pass("✅ PASS verify loại hàng giảm giá 7000đ-KM-0626-071 SP C042300000097. Mã đơn: " + orderCode);
    }
}
